...
//  Loopsone
//
//  Created by UI on 2017/1/14.
//  Copyright © 2017年 Kingsingmobi. All rights reserved.
//

#import "UIView+QXToast.h"

#import <QuartzCore/QuartzCore.h>
#import <objc/runtime.h>
const int32_t QXToastTag = 1234;
...
static const NSString * CSToastMessageKey = @"CSToastMessageKey";
...
@implementation UIView (QXToast)

#pragma mark - Make Toast Methods
- (void)showIndicatorWithCompletion:(void (^)())completion {
    UIActivityIndicatorView *indicator = [[UIActivityIndicatorView alloc] initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleWhite];
    [indicator startAnimating];
    __weak UIActivityIndicatorView *weakIndicator = indicator;
    [self showToast:indicator duration:[CSToastManager defaultDuration] position:[CSToastManager defaultPosition] completion:^(BOOL didTap) {
...
- (UIActivityIndicatorView*)showIndicator {
    if (self.indicatorView) {
        ...
        return (UIActivityIndicatorView*)self.indicatorView;
    }
    UIActivityIndicatorView *indicator = [[UIActivityIndicatorView alloc] initWithActivityIndicatorStyle:UIActivityIndicatorViewStyleGray];
    
    self.indicatorView = indicator;
    [indicator startAnimating];
    [self showToast:indicator duration:CGFLOAT_MAX position:[CSToastManager defaultPosition]  completion:nil];
    return indicator;
}

- (void)hideIndicator {
    [self hideToast:self.indicatorView];
...
- (void)makeToast:(NSString *)message {
    if (message && message.length > 0) {
        ...
        style.backgroundColor = [UIColor colorWithRed:48.0 / 255.0 green:209.0 / 255.0 blue:139.0 / 255.0 alpha:1];
        style.messageFont = [UIFont systemFontOfSize:14 weight:UIFontWeightMedium];
        style.messageAlignment = NSTextAlignmentCenter;
        style.cornerRadius = 16;
        style.horizontalPadding = 24;
        style.verticalPadding = 7.5;
        style.shadowRadius = 12;
        style.displayShadow = true;
        style.shadowColor = [[UIColor blackColor] colorWithAlphaComponent:0.2];
        style.shadowOpacity = 1;
        style.shadowOffset = CGSizeZero;
        style.fadeDuration = 0.4;
        CGFloat top = 0;
        if (@available(iOS 11.0, *)) {
            top = [UIApplication sharedApplication].keyWindow.safeAreaInsets.top > 20 ? 20 : 0;
        }
        CGRect rect = [self.superview convertRect:self.frame toView:UIApplication.sharedApplication.keyWindow];
        CGPoint p = CGPointMake(self.frame.size.width / 2, 92 - rect.origin.y + top);
        [self makeToast:message duration:[CSToastManager defaultDuration] position:[NSValue valueWithCGPoint:p] style:style];
    }
}
...
- (void)makeToast:(NSString *)message duration:(NSTimeInterval)duration position:(id)position style:(CSToastStyle *)style {
    ...
    toast.tag = QXToastTag;
    BOOL repeatedMsg = NO;
    
    for (UIView *view in [self cs_activeToasts]) {
        
        NSString* msg = objc_getAssociatedObject(view, &CSToastMessageKey);
    ...
    for (UIView *view in [self cs_toastQueue]) {
    ...
    if (!repeatedMsg) {
        objc_setAssociatedObject(toast, &CSToastMessageKey, message, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
- (void)cs_showToast:(UIView *)toast duration:(NSTimeInterval)duration position:(id)position {
    ...
    //
    CGPoint oriCenter = toast.center;
    toast.center = CGPointMake(oriCenter.x, oriCenter.y - 20);
    [UIView animateWithDuration:[[CSToastManager sharedStyle] fadeDuration]
                          delay:0.0
                        options:(UIViewAnimationOptionCurveEaseOut | UIViewAnimationOptionAllowUserInteraction)
                     animations:^{
        toast.center = oriCenter;
                         toast.alpha = 1.0;
...
- (void)cs_hideToast:(UIView *)toast fromTap:(BOOL)fromTap {
    [UIView animateWithDuration:[[CSToastManager sharedStyle] fadeDuration]
                     ...
                     animations:^{
        toast.center = CGPointMake(toast.center.x, toast.center.y - 20);
                         toast.alpha = 0.0;
                     } completion:^(BOOL finished) {
                         [toast removeFromSuperview];
                         
                         NSTimer *timer = (NSTimer *)objc_getAssociatedObject(toast, &CSToastTimerKey);
                         [timer invalidate];// 有可能前面 那次取消的时候才 刚创建，也就没有取消成功、这里再取消一次。
...
-(UIView*)toastViewForAttributedMessage:(NSAttributedString*)message{
...
-(void)setIndicatorView:(UIView *)indicatorView{
    ...
    objc_setAssociatedObject(self, @selector(setIndicatorView:), indicatorView, OBJC_ASSOCIATION_RETAIN_NONATOMIC);
...
-(UIView*)indicatorView{
    ...
    return objc_getAssociatedObject(self, @selector(setIndicatorView:));
}
...
- (instancetype)initWithDefaultStyle {
    ...
    if (self) {
        ...
        self.titleColor = [UIColor colorWithRed:243/255.0 green:243/255.0 blue:243/255.0 alpha:1];
        self.messageColor = [UIColor whiteColor];
        self.maxWidthPercentage = 0.8;
        self.maxHeightPercentage = 0.8;
        self.horizontalPadding = 30;
        self.verticalPadding = 18;
